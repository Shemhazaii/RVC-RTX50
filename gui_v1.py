...
if __name__ == "__main__":
    ...
    class GUI:
        ...
        def load(self):
            ...
            try:
                if os.path.exists(config_path) and os.path.getsize(config_path) > 0:
                    ...
                    raise ValueError("config empty or missing")

            except Exception as e:
                print("Config load failed, using default:", e)
                data = {}

            # DEFAULT DEVICE INDEX SAFE
            def safe_device(dev_list, idx):
                if not dev_list:
                    return ""
                if idx is None:
                    return dev_list[0]
                if idx >= len(dev_list) or idx < 0:
                    return dev_list[0]
                return dev_list[idx]
            ...
            # clamp index safety
            in_idx = in_idx if isinstance(in_idx, int) else 0
            out_idx = out_idx if isinstance(out_idx, int) else 0

            if in_idx >= len(input_devices) or in_idx < 0:
                in_idx = 0
            if out_idx >= len(output_devices) or out_idx < 0:
                out_idx = 0

            data["sg_input_device"] = safe_device(input_devices, in_idx)
            data["sg_output_device"] = safe_device(output_devices, out_idx)

            # ensure f0 flags always exist
            f0method = data.get("f0method", "rmvpe")
            data["pm"] = f0method == "pm"
            data["harvest"] = f0method == "harvest"
            data["crepe"] = f0method == "crepe"
            data["rmvpe"] = f0method == "rmvpe"
                        ...
                        layout=[
                            [
                                ...
                                sg.FileBrowse(
                                    ...
                                    file_types=(("PTH Files", "*.pth"), ("All Files", "*.*")),
                                ),
                            ],
                            [
                                sg.Input(
        ...
        def set_devices(self, input_device, output_device):
            ...
            try:
                in_idx = input_devices.index(input_device)
            except:
                in_idx = 0

            try:
                out_idx = output_devices.index(output_device)
            except:
                out_idx = 0

            sd.default.device = [
                input_device_indices[in_idx],
                output_device_indices[out_idx],
            ]
